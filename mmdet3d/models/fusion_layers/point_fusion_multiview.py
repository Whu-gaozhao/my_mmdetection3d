...
class PointFusionMultiview(BaseModule):
    ...
    def __init__(self,
                 ...
                 padding_mode='zeros',
                 lateral_conv=True):
        super(PointFusionMultiview, self).__init__(init_cfg=init_cfg)
        ...
        if lateral_conv:
            ...
            self.img_transform = nn.Sequential(
                nn.Conv1d(mid_channels * len(img_channels), out_channels, 1),
                nn.BatchNorm1d(out_channels, eps=1e-3, momentum=0.01),
            )
        else:
            self.img_transform = nn.Sequential(
                nn.Conv1d(sum(img_channels), out_channels, 1),
                nn.BatchNorm1d(out_channels, eps=1e-3, momentum=0.01),
            )
        self.pts_transform = nn.Sequential(
            nn.Conv1d(pts_channels, out_channels, 1),
            nn.BatchNorm1d(out_channels, eps=1e-3, momentum=0.01),
        )
